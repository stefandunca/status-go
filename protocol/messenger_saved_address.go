...
func (m *Messenger) AddSavedAddress(ctx context.Context, sa wallet.SavedAddress) error {
	updatedClock, err := m.savedAddressesManager.UpsertSavedAddress(sa)
	if err != nil {
		return err
	}
...
func (m *Messenger) garbageCollectRemovedSavedAddresses() error {
	return m.savedAddressesManager.DeleteSoftRemovedSavedAddresses(time.Now().AddDate(0, 0, -30).Unix())
}
...
func (m *Messenger) SyncNewSavedAddress(ctx context.Context, savedAddress *wallet.SavedAddress, updateClock int64) error {
...
func (m *Messenger) SyncDeletedSavedAddress(ctx context.Context, chainID uint64, address gethcommon.Address, updateClock int64) error {
	return m.dispatchSyncSavedAddress(ctx, protobuf.SyncSavedAddress{
...
func (m *Messenger) handleSyncSavedAddress(state *ReceivedMessageState, syncMessage protobuf.SyncSavedAddress) (err error) {
	...
	if syncMessage.Removed {
		_, err = m.savedAddressesManager.DeleteSavedAddressIfNewerUpdate(syncMessage.ChainId,
			address, int64(syncMessage.SyncClock), syncMessage.UpdateClock)
		if err != nil {
			return err
		}
		...
		_, err = m.savedAddressesManager.AddSavedAddressIfNewerUpdate(sa, int64(syncMessage.SyncClock), syncMessage.UpdateClock)
		if err != nil {
			return err
		}
